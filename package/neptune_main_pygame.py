...
def main():
    ...
    icon = pygame.image.load(
        os.path.join(resources.RESOURCES_DIRECTORY,'icons/Chell_Logo.ico'))
    pygame.display.set_icon(icon)

    infoObject = pygame.display.Info()
    W = infoObject.current_w
    H = infoObject.current_h - 40
    display = (W, H)
    screen = pygame.display.set_mode(display, DOUBLEBUF | OPENGL | SRCALPHA | NOFRAME)
    pygame.display.set_caption("My Little Neptune")
    winapi = WinApiClient()
    ...
    if live2d.LIVE2D_VERSION == 3:
        model.LoadModelJson(
            os.path.join(resources.RESOURCES_DIRECTORY, "v3/Nep_Models/Neptune/Neptune.model3.json")
        )
    else:
        model.LoadModelJson(
            os.path.join(resources.RESOURCES_DIRECTORY, "v2/NeptuneHappinessSanta/neptune_m_model_c031.json")
        )
